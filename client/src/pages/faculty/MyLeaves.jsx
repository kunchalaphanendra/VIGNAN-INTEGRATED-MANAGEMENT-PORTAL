...
export default function FacultyMyLeaves() {
    ...
    useEffect(() => { load(); }, []);
    const load = async () => { try { const r = await api.get('/faculty/faculty-leaves'); setLeaves(r.data.leaves); } catch { } finally { setLoading(false); } };
    const handleSubmit = async (e) => {
        e.preventDefault();
        try { await api.post('/faculty/faculty-leaves', form); setShowModal(false); load(); } catch (err) { alert(err.response?.data?.error || 'Error'); }
    };
        ...
        <DashboardLayout>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                <div>
                    ...
                    <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', marginTop: 2 }}>Apply for leave and track approvals</p>
                </div>
                <button onClick={() => setShowModal(true)} className="btn btn-faculty">+ Request Leave</button>
            </div>
            ...
            <Modal isOpen={showModal} onClose={() => setShowModal(false)} title="Request Leave" size="sm">
                <form onSubmit={handleSubmit} className="modal-form">
                    ...
                    <div className="modal-footer">
                        ...
                        <button type="submit" className="btn btn-faculty">Submit Request</button>
                    </div>
                </form>
            </Modal>
        </DashboardLayout>
